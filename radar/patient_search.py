...
class PatientQueryBuilder(object):
    ...
    def patient_diagnosis(self, diagnosis_code):
        self.query = self.query.filter(filter_by_patient_diagnosis(diagnosis_code))
        return self
...
def filter_by_patient_diagnosis(diagnosis_code):
    ...
    if code:
        code_id = code.id
        diagnosis = (
            db.session.query(DiagnosisCode)
            .filter(DiagnosisCode.code_id == code_id)
            .first()
        )
        if diagnosis:
