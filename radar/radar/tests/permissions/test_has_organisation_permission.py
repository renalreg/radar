from radar.permissions import has_organisation_permission
...
def test_admin():
    ...
    assert not has_organisation_permission(user, organisation, 'has_view_patient_permission')

    user.is_admin = True

    assert has_organisation_permission(user, organisation, 'has_view_patient_permission')
...
def test_not_in_organisation():
    ...
    assert has_organisation_permission(user, organisation_a, 'has_view_patient_permission')
    assert not has_organisation_permission(user, organisation_b, 'has_view_patient_permission')
...
def test_in_cohort_without_permission():
    ...
    assert not has_organisation_permission(organisation_it_user, organisation, 'has_view_demographics_permission')
    assert has_organisation_permission(organisation_clinician_user, organisation, 'has_view_demographics_permission')
