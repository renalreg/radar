...
def build(c, rev="HEAD"):
    ...
    c.local('git archive "{rev}" | gzip > "{archive}"'.format(rev=rev, archive=archive))

    with temp(c) as working_dir:
        c.put(archive, os.path.join(working_dir, "src.tar.gz"))
        c.run("tar -xzf src.tar.gz")
        c.run(
            "PATH=/usr/pgsql-9.4/bin:$PATH platter build -p python3 -r requirements.txt ."
        )
        if not os.path.exists(DEFAULT_DIST_DIR):
            os.mkdir(DEFAULT_DIST_DIR)
        res = c.run("ls dist")
        fname = res.stdout.strip()
        c.get(
            os.path.join(working_dir, "dist", fname),
            os.path.join(DEFAULT_DIST_DIR, fname),
        )
