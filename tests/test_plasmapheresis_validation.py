...
def test_plasmapheresis_valid(plasmapheresis):
    valid(plasmapheresis)


def test_plasmapheresis_from_date_missing(plasmapheresis):
    plasmapheresis.from_date = None
    invalid(plasmapheresis)


def test_plasmapheresis_from_date_before_dob(plasmapheresis):
    plasmapheresis.from_date = date(1999, 1, 1)
    invalid(plasmapheresis)


def test_plasmapheresis_from_date_future(plasmapheresis):
    plasmapheresis.from_date = date.today() + timedelta(days=1)
    invalid(plasmapheresis)


def test_plasmapheresis_to_date_missing(plasmapheresis):
    plasmapheresis.to_date = None
    valid(plasmapheresis)


def test_plasmapheresis_to_date_before_dob(plasmapheresis):
    plasmapheresis.to_date = date(1999, 1, 1)
    invalid(plasmapheresis)


def test_plasmapheresis_to_date_future(plasmapheresis):
    plasmapheresis.to_date = date.today() + timedelta(days=1)
    invalid(plasmapheresis)


def test_plasmapheresis_to_date_before_from_date(plasmapheresis):
    plasmapheresis.to_date = plasmapheresis.from_date - timedelta(days=1)
    invalid(plasmapheresis)


def test_plasmapheresis_no_of_exchanges_missing(plasmapheresis):
    plasmapheresis.no_of_exchanges = None
    invalid(plasmapheresis)


def test_plasmapheresis_no_of_exchanges_zero(plasmapheresis):
    plasmapheresis.no_of_exchanges = 0
    valid(plasmapheresis)


def test_plasmapheresis_response_missing(plasmapheresis):
...
def validate(plasmapheresis):
    ...
    validation.after_update(ctx, old_obj, plasmapheresis)
